...
// Même catalogue/prix que Kinuma (tu peux diverger si besoin)
const CATALOG = [
...
module.exports = (client) => {
  client.once('ready', async () => {
    const catChannelId = process.env.HOCKLEY_STABLE_CHANNEL;           // ⬅️ NOUVEAU salon catalogue Hockley
    const orderLogChannelId = process.env.HOCKLEY_COMMANDER_CHEVAL_CHANNEL; // ⬅️ NOUVEAU salon log Hockley

    if (!catChannelId) return console.error('HOCKLEY_STABLE_CHANNEL non défini.');
    const ch = await client.channels.fetch(catChannelId).catch(() => null);
    if (!ch || ch.type !== ChannelType.GuildText) return console.error('Salon Hockley’s Horse introuvable / non textuel');

    const msgs = await ch.messages.fetch({ limit: 30 }).catch(() => null);
    if (msgs?.some(m => m.embeds[0]?.title === 'Catalogue — Hockley’s Horse')) return;
    ...
    const embed = new EmbedBuilder()
      .setColor(0xE67E22)
      .setTitle('Catalogue — Hockley’s Horse')
      .setDescription(lines.join('\n'));

    await ch.send({ embeds: [embed] });

    // Un message par catégorie avec un menu propre à Hockley
    for (const group of CATALOG) {
      const menu = new StringSelectMenuBuilder()
        .setCustomId('horse_order_select_hockley') // ≠ de Kinuma
    ...
    // (Optionnel) petit handler local si tu n’utilises pas l’handler d’interactions dédié
    client.on('interactionCreate', async (interaction) => {
      if (!interaction.isStringSelectMenu()) return;
      if (interaction.customId !== 'horse_order_select_hockley') return;
      ...
      if (logCh) {
        await logCh.send(`🐎 (Hockley) Vous avez fabriqué **${chosen}** (par ${interaction.user}).`).catch(() => {});
      }
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({ content: `✅ **${chosen}** enregistré pour Hockley.`, flags: MessageFlags.Ephemeral }).catch(() => {});
      }
    });
  });
};
